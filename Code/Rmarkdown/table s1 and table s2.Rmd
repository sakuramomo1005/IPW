...
```{R echo=FALSE}
kable(table_sum, "latex", longtable = T, booktabs = T,
caption = 'Table S1: Results of analysis of high literacy outcome from HALI motivating data set using five different GEE methods (with robust SE)') %>%
  add_header_above(c("", "Exchangeable" = 4, 
                   "Independent" = 4
                   )) %>%
...
kable(tableS2, "latex", booktabs=TRUE, escape = F,    row.names=FALSE, longtable = T,linesep=linesep,
      caption = 'Table S2: Predictors of outcome (high literacy) and of missingness (of high literacy outcome) from HALI motivating data set',
col.names = linebreak(c(' ','Estimate','Std.Error','Pr(>|z|)','Estimate','Std.Error','Pr(>|z|)'))) %>%
kable_styling(latex_options = c("repeat_header")) %>%
add_indent(c(7:9,11:14))
```
